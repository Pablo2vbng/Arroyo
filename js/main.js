...
form.addEventListener('submit', function(event) {
    ...
    const fileInputs = [ document.getElementById('fotoDelantera'), document.getElementById('fotoTrasera'), document.getElementById('fotoDetalleDefecto'), document.getElementById('fotoEtiqueta') ];
...
async function generatePdfAndShowConfirmation(data, images) {
    ...
    try {
        ...
        try { const upowerLogoBase64 = await imageToBase64('img/upower.png'); doc.addImage(upowerLogoBase64, 'PNG', margin, 5, 25, 10); } catch (logoError) { console.warn('Logo no encontrado.'); }
        ...
        // Configuramos los botones de confirmación
        const formContainer = document.getElementById('formContainer');
        const confirmationSection = document.getElementById('confirmationSection');
        const finalPdfLink = document.getElementById('finalPdfLink');
        const finalMailtoLink = document.getElementById('finalMailtoLink');

        // Asignamos la URL del blob directamente al botón de ver PDF
        finalPdfLink.href = blobUrl;
        
        // Preparamos el mailto
        ...
        finalMailtoLink.href = `mailto:nacho@representacionesarroyo.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
