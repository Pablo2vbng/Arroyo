...
form.addEventListener('submit', function(event) {
    ...
    // ===== CAMBIO CLAVE 1: Abrimos la pestaña en blanco INMEDIATAMENTE =====
    const pdfTab = window.open('', '_blank');
    if (!pdfTab) {
        alert("Por favor, permite las ventanas emergentes para poder ver el PDF.");
        resetButtonState();
        return; // Detenemos la ejecución si el pop-up fue bloqueado
    }
    pdfTab.document.write('Generando PDF, por favor espera...'); // Mensaje temporal en la nueva pestaña
    ...
    Promise.all(filePromises)
        .then(imagesBase64 => {
            ...
            // Pasamos la referencia de la nueva pestaña a la función que genera el PDF
            generatePdfAndShowConfirmation(data, images, pdfTab);
        })
        .catch(error => {
            ...
            pdfTab.close(); // Cerramos la pestaña vacía si hay un error
            resetButtonState();
        });
});

async function generatePdfAndShowConfirmation(data, images, pdfTab) {
    ...
    try {
        ...
        // --- (TODA LA LÓGICA DE DIBUJO DEL PDF SIGUE IGUAL) ---
        doc.setFontSize(14);
        ...
        // ===== CAMBIO CLAVE 2: Rellenamos la pestaña que ya está abierta =====
        const pdfDataUri = doc.output('datauristring');
        pdfTab.location.href = pdfDataUri; // Asignamos el PDF a la pestaña vacía

        // Mostramos el mensaje de confirmación en la página original
